        ...
        for cell in self.cellList:
            if cell.type == 1:
                ...
                if cell.targetVolume:
                    if cell.yCOM < self.dim.y*0.3:# only stem cell zone is growing fast
                    # Program Cell Growth
                        cell.targetVolume+= 1.5   
                    # The diff cells remain unchanged
                    elif cell.yCOM>self.dim.y*0.3 and cell.yCOM<self.dim.y*0.5:# transit amplifying zone
...
class DeltaNotchClass(SteppableBasePy):
    ...
    def start(self):
        ...
        for cell in self.cellListByType(1):
            state['D'] = random.uniform(0.8,1.0)
            state['N'] = random.uniform(0.8,1.0)
...
class MitosisSteppable(MitosisSteppableBase):
    ...
    def step(self,mcs):
        ...
        for cell in self.cellListByType(1):
            cellDict=self.getDictionaryAttribute(cell)
            if mcs>200 and cellDict['N']>0.8 and cell.yCOM<self.dim.y*0.3 and cell.volume > tVol*1.5:
